...
from src.detector import Detector  # Assume this defines a base Detector class
...
class DebugType(Enum):
    ...
    WATERSHED_SEG_SINGLE_COLOR = "watershed_seg_single_color"
...
class WatershedDetector(Detector):
    ...
    def process_frame(self, frame: img_t) -> List[Block]:
        ...
        preprocessed = self.preproc.process(frame)

        # 2) Convert to HSV
        hsv = cv2.cvtColor(preprocessed, cv2.COLOR_BGR2HSV)

        # 3) Detect blocks using watershed
    ...
    def _detect_blocks(self, hsv: img_t, frame_bgr: img_t) -> List[Block]:
        ...
        for color_def in self.detecting_colors:
            ...
            markers, num_labels, sure_bg, sure_fg, unknown = self._watershed_segment(
                color_mask, color_def.name, frame_bgr) # TODO: add color suffix for debug images
    ...
    def _watershed_segment(
        ...
        if DebugType.WATERSHED_BG in self.debug_option:
        ...
        if DebugType.WATERSHED_DIST_TRANSFORM in self.debug_option:
        ...
        if DebugType.WATERSHED_FG in self.debug_option:
        ...
        if DebugType.WATERSHED_UNKNOWN in self.debug_option:
        ...
        if DebugType.WATERSHED_SEG  in self.debug_option:
            h, w = mask.shape[:2]
            ...
            for lbl in range(2, num_labels + 1):
                ...
                # Get the current region's pixels as float for alpha blending
                region_pixels = seg_vis[region_mask].astype(np.float32)

                # Blend with the label color
                blended = region_pixels * (1.0 - alpha) + \
                    label_colors[lbl].astype(np.float32) * alpha

                # Convert back to uint8 and assign back
                seg_vis[region_mask] = blended.astype(np.uint8)
    ...
    def create_color_mask(self, frame_hsv: np.ndarray, colors: List[Color] | Color) -> np.ndarray:
        mask = super().create_color_mask(frame_hsv, colors)
        # enlarge the mask by dialation 
